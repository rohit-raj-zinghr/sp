...
BEGIN
    ...
    -- Calculate shift window as DATETIME for Punch-In validation
    ...
    IF NOT EXISTS (
        ...
        AND SwipeDate BETWEEN @ValidPunchInStart AND DATEADD(MINUTE, @PostTime, @ShiftEndDateTime)
        AND TermNo = 'Punch IN'
    )
    BEGIN
        SET @IsPunchIn = 1;
    ...
    -- Validate punch timing (only for Punch-In)
    IF @IsPunchIn = 1
    BEGIN
        ...
        END;
    END
    ELSE -- Punch-Out: No timing restriction, just shift verification
    BEGIN
        SET @IsValidPunch = 1; -- Valid as long as shift exists and other validations pass
    END;
...
EXEC dbo.ValidateEmployeePunch
    @EmpCode = 'admin',
    @PunchDateTime = '2025-03-11 08:10:00', -- UTC, becomes 14:15:00 IST
    @Latitude = 19.533572,
    @Longitude = 78.877306,
