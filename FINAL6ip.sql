CREATE OR ALTER PROCEDURE dbo.CheckEmployeePunch
    @EmpCode VARCHAR(100),
...
BEGIN
    ...
    BEGIN
        IF @IPRangeJson IS NOT NULL AND @IPRangeJson != '[]'
        BEGIN
            IF ISJSON(@IPRangeJson) = 1
            BEGIN
        ...
        ELSE
        BEGIN
            SET @Message = 'No IP ranges found for employee ' + @EmpCode + ' in FlatTable.';
            RETURN;
        END;
    END
    ELSE
...
EXEC dbo.CheckEmployeePunch
    @EmpCode = 'rohit',
    @PunchDateTime = '2025-03-14 10:00:00',
    @Latitude = 19.533572,
    @Longitude = 78.877306,
